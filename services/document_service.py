...
class DocumentService:
    @staticmethod
    def create_doc(doc_id: str, title: str, mcb: str, age_category: str, developer: str, creator: str, file_data: bytes):
        data_base.add_to_upload_list(doc_id, title, mcb, age_category, developer, creator, data=file_data)
    ...
    @staticmethod
    def get_my_docs(creator: str, page: int = 0, size: int = 10) -> List[Dict]:
        return data_base.get_my_docs_paginated(creator, page=page, size=size)

    @staticmethod
    def delete_my_doc(login, doc_id: str):
        ans = data_base.is_client_doc(login, doc_id)
        if ans:
            data_base.delete_data((doc_id,))
            return True
        return False
